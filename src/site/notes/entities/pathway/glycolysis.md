---
created: '2026-02-08T19:29:31.728635Z'
description: Glycolysis is the fundamental metabolic pathway that breaks down glucose
  to pyruvate, generating ATP and NADH. It is the central route for glucose metabolism
  in virtually all organisms.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/pathway/glycolysis/
slug: glycolysis
tags:
- pathway
- metabolic
- energy_metabolism
- glucose_metabolism
- catabolic
title: Glycolysis
type: pathway
updated: '2026-02-08T19:29:31.728635Z'
---

# Glycolysis

## Description

Glycolysis is the fundamental metabolic pathway that breaks down glucose to pyruvate, generating ATP and NADH. It is the central route for glucose metabolism in virtually all organisms.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[ATP]] (metabolite)
```yaml
last_modified: '2026-01-26T06:48:40.986199'
source: obsidian
```
- ← [[ATP]] (metabolite)
```yaml
last_modified: '2026-01-26T07:08:05.909377'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships

No relationships found.