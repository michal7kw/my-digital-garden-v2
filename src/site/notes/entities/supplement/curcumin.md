---
created: '2026-02-08T19:45:44.652119Z'
description: Active compound in turmeric with powerful anti-inflammatory and antioxidant
  properties. Supports joint health, brain function, and metabolic health. Requires
  enhanced absorption formulations.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/supplement/curcumin/
slug: curcumin
tags:
- supplement
- polyphenol
- anti_inflammatory
- antioxidant
- joint_health
- brain_health
title: Curcumin
type: supplement
updated: '2026-02-08T19:45:44.652119Z'
---

# Curcumin

## Overview
Active compound in turmeric with powerful anti-inflammatory and antioxidant properties. Supports joint health, brain function, and metabolic health. Requires enhanced absorption formulations.








<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[BDNF]] (biomarker)
```yaml
last_modified: '2026-01-26T06:42:49.757693'
source: obsidian
```
- → [[AMPK]] (protein)
```yaml
last_modified: '2026-01-26T06:42:49.757693'
source: obsidian
```
- → [[Hs_Crp]] (entity)
```yaml
last_modified: '2026-01-26T06:42:49.757693'
source: obsidian
```
- → [[NRF2]] (protein)
```yaml
last_modified: '2026-01-26T06:42:49.757693'
source: obsidian
```
- → [[Neuroinflammation]] (pathway)
```yaml
last_modified: '2026-01-26T06:42:49.757693'
source: obsidian
```
- → [[Omega-3_DHA]] (entity)
```yaml
last_modified: '2026-01-26T06:42:49.757693'
source: obsidian
```
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-26T07:08:36.742492'
source: obsidian
```
- → [[Omega-3_Fatty_Acids]] (entity)
```yaml
last_modified: '2026-01-26T07:08:36.742492'
source: obsidian
```
- → [[Metabolic_Syndrome]] (entity)
```yaml
last_modified: '2026-01-26T07:08:36.742492'
source: obsidian
```
- ← [[Omega-3_DHA]] (supplement)
```yaml
last_modified: '2026-01-26T07:08:39.685849'
source: obsidian
```
- ← [[NRF2]] (protein)
```yaml
last_modified: '2026-01-26T07:08:31.690636'
source: obsidian
```
- ← [[AMPK]] (protein)
```yaml
last_modified: '2026-01-26T07:08:28.642108'
source: obsidian
```
- ← [[Neuroinflammation]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:12.428247'
source: obsidian
```
- ← [[Hs_Crp]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:13.822195'
source: obsidian
```
- ← [[BDNF]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:10.191066'
source: obsidian
```
- ← [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2026-01-26T07:08:39.989104'
source: obsidian
```
- → [[Turmeric]] (drug)
```yaml
last_modified: '2026-01-26T07:08:36.742492'
source: obsidian
```
- → [[Liver]] (organ)
```yaml
last_modified: '2026-01-26T07:08:36.742492'
source: obsidian
```
- → [[Joints]] (organ)
```yaml
last_modified: '2026-01-26T07:08:36.742492'
source: obsidian
```
- → [[Brain]] (organ)
```yaml
last_modified: '2026-01-26T07:08:36.742492'
source: obsidian
```
- ← [[Liver]] (organ)
```yaml
last_modified: '2026-01-26T07:08:09.492150'
source: obsidian
```
- ← [[Joints]] (organ)
```yaml
last_modified: '2026-01-26T07:08:09.332150'
source: obsidian
```
- ← [[Brain]] (organ)
```yaml
last_modified: '2026-01-26T07:08:07.961287'
source: obsidian
```
- ← [[Turmeric]] (drug)
```yaml
last_modified: '2026-01-26T07:07:43.944079'
source: obsidian
```
- ← [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2026-01-26T07:07:32.678592'
source: obsidian
```
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```

### Improved By Supplement
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```

### Synergistic With
- ← [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:24.619060'
source: obsidian
```
- → [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:20.437235'
source: obsidian
```

### Affects Organ
- → [[Brain]] (organ)
```yaml
last_modified: '2025-12-28T11:01:20.291386'
source: obsidian
```
- → [[Joints]] (organ)
```yaml
last_modified: '2025-12-28T11:01:20.321396'
source: obsidian
```
- → [[Liver]] (organ)
```yaml
last_modified: '2025-12-28T11:01:20.266685'
source: obsidian
```

### Contains
- ← [[Turmeric]] (drug)
```yaml
last_modified: '2025-12-28T11:01:09.447198'
source: obsidian
```

### May Improve Condition
- → [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2025-12-28T11:01:20.562067'
source: obsidian
```
- → [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2025-12-28T11:01:20.513724'
source: obsidian
```

### Affects Biomarker
- → [[Hs Crp]] (biomarker)
```yaml
effect_direction: decreases
magnitude: moderate
evidence_level: 4
source: curated
```
- → [[Esr]] (biomarker)
```yaml
effect_direction: decreases
magnitude: mild
evidence_level: 3
source: curated
```
<!-- NEO4J_CONTENT_END -->
## Relationships
### Affects Organ
- → [[Liver]] (organ)
```yaml
last_modified: '2025-12-28T11:01:20.266685'
source: obsidian
```
- → [[Joints]] (organ)
```yaml
last_modified: '2025-12-28T11:01:20.321396'
source: obsidian
```
- → [[Brain]] (organ)
```yaml
last_modified: '2025-12-28T11:01:20.291386'
source: obsidian
```

### May Improve Condition
- → [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2025-12-28T11:01:20.513724'
source: obsidian
```
- → [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2025-12-28T11:01:20.562067'
source: obsidian
```

### Contains
- ← [[Turmeric]] (drug)
```yaml
last_modified: '2025-12-28T11:01:09.447198'
source: obsidian
```

### Synergistic With
- → [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:20.437235'
source: obsidian
```
- ← [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:24.619060'
source: obsidian
```

### Related
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-19T09:07:02.331491'
source: obsidian
```
- → [[Metabolic_Syndrome]] (entity)
```yaml
last_modified: '2026-01-19T09:07:02.331491'
source: obsidian
```
- → [[Omega-3_Fatty_Acids]] (entity)
```yaml
last_modified: '2026-01-19T09:07:02.331491'
source: obsidian
```
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- ← [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2026-01-19T09:31:23.144461'
source: obsidian
```
- ← [[Turmeric]] (drug)
```yaml
last_modified: '2026-01-19T09:32:04.933735'
source: obsidian
```
- ← [[Brain]] (organ)
```yaml
last_modified: '2026-01-19T09:33:02.002935'
source: obsidian
```
- ← [[Joints]] (organ)
```yaml
last_modified: '2026-01-19T09:33:07.218935'
source: obsidian
```
- ← [[Liver]] (organ)
```yaml
last_modified: '2026-01-19T09:33:07.798545'
source: obsidian
```
- → [[Brain]] (organ)
```yaml
last_modified: '2026-01-19T09:07:02.331491'
source: obsidian
```
- → [[Joints]] (organ)
```yaml
last_modified: '2026-01-19T09:07:02.331491'
source: obsidian
```
- → [[Liver]] (organ)
```yaml
last_modified: '2026-01-19T09:07:02.331491'
source: obsidian
```
- → [[Turmeric]] (drug)
```yaml
last_modified: '2026-01-19T09:07:02.331491'
source: obsidian
```
- ← [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2026-01-19T09:07:12.965838'
source: obsidian
```

### Improved By Supplement
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```