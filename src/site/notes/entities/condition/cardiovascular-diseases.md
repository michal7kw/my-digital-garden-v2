---
created: '2026-02-08T19:45:44.465793Z'
description: A group of disorders of the heart and blood vessels, including coronary
  heart disease, cerebrovascular disease, and rheumatic heart disease.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/condition/cardiovascular-diseases/
slug: cardiovascular-diseases
tags:
- condition
- cardiology
- vascular
- chronic
- global_health
title: Cardiovascular Diseases
type: condition
updated: '2026-02-08T19:45:44.465793Z'
---

# Cardiovascular Diseases

## Overview

Cardiovascular Diseases (CVDs) are the leading cause of death globally. They encompass a wide range of conditions affecting the heart and blood vessels. **Atherosclerosis** (plaque buildup in arteries) is the underlying cause of many CVDs.

## Common Types

- **Coronary Heart Disease (CHD):** Disease of the blood vessels supplying the heart muscle.
- **Cerebrovascular Disease:** Disease of the blood vessels supplying the brain (Stroke).
- **Peripheral Arterial Disease:** Disease of blood vessels supplying arms and legs.
- **Rheumatic Heart Disease:** Damage to the heart muscle and valves from rheumatic fever.
- **Congenital Heart Disease:** Malformations of heart structure existing at birth.
- **Deep Vein Thrombosis and Pulmonary Embolism.**

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[Cardiovascular_Diseases]] (entity)
```yaml
last_modified: '2026-01-26T06:47:33.760330'
source: obsidian
```
- → [[Cardiovascular_Diseases]] (condition)
```yaml
last_modified: '2026-01-26T06:03:28.538943'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships

CAUSED_BY::[[Atherosclerosis]]
ASSOCIATED_WITH_CONDITION::[[Hypertension]]
ASSOCIATED_WITH_CONDITION::[[Type 2 Diabetes]]
ASSOCIATED_WITH_CONDITION::[[Obesity]]
ASSOCIATED_WITH_BIOMARKER::[[LDL Cholesterol]]
ASSOCIATED_WITH_BIOMARKER::[[Triglycerides]]
AFFECTS_ORGAN::[[Heart]]
AFFECTS_ORGAN::[[Blood Vessel]]
TREATED_BY_DRUG::[[Atorvastatin]] - Statin.
TREATED_BY_DRUG::[[Lisinopril]] - ACE Inhibitor.
TREATED_BY_DRUG::[[Aspirin]] - Antiplatelet.

## References

- World Health Organization. Cardiovascular diseases (CVDs).
- American Heart Association. What is Cardiovascular Disease?
- Benjamin, E. J., et al. (2019). Heart disease and stroke statistics—2019 update. Circulation.