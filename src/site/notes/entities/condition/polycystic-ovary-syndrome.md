---
created: '2026-02-09T05:54:32.100612Z'
description: A hormonal disorder common among women of reproductive age, characterized
  by irregular periods, excess androgen levels, and cysts on the ovaries.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/condition/polycystic-ovary-syndrome/
slug: polycystic-ovary-syndrome
tags:
- condition
- endocrinology
- gynecology
- metabolic
- hormonal
templateEngineOverride: njk
title: Polycystic Ovary Syndrome
type: condition
updated: '2026-02-09T05:54:32.100612Z'
---

{% raw %}
<h1>Polycystic Ovary Syndrome</h1>
<h2>Overview</h2>
<p>Polycystic Ovary Syndrome (PCOS) is a hormonal imbalance that affects ovulation. The ovaries may develop numerous small collections of fluid (follicles) and fail to regularly release eggs. It is a leading cause of infertility.</p>
<h2>Complications</h2>
<ul>
<li><strong>Infertility.</strong></li>
<li><strong>Metabolic Syndrome:</strong> High blood pressure, high blood sugar, abnormal cholesterol.</li>
<li><strong>Type 2 Diabetes.</strong></li>
<li><strong>Sleep Apnea.</strong></li>
<li><strong>Endometrial Cancer.</strong></li>
</ul>
<!-- NEO4J_CONTENT_START -->
<h2>Relationships</h2>
<h3>Related</h3>
<ul>
<li>→ [[Polycystic_Ovary_Syndrome]] (entity)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T06:47:50.628336'
source: obsidian
</code></pre>
<ul>
<li>→ [[Polycystic_Ovary_Syndrome]] (condition)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T06:03:54.910570'
source: obsidian
</code></pre>
<!-- NEO4J_CONTENT_END -->
<h2>Relationships</h2>
<p>CAUSED_BY::[[Insulin Resistance]]<br />
ASSOCIATED_WITH_CONDITION::[[Infertility]]<br />
ASSOCIATED_WITH_CONDITION::[[Type 2 Diabetes]]<br />
ASSOCIATED_WITH_CONDITION::[[Obesity]]<br />
ASSOCIATED_WITH_BIOMARKER::[[Testosterone]] - Elevated.<br />
ASSOCIATED_WITH_BIOMARKER::[[Insulin]]<br />
AFFECTS_ORGAN::[[Ovary]]<br />
TREATED_BY_DRUG::[[Metformin]]<br />
TREATED_BY_DRUG::[[Spironolactone]] - For hirsutism.<br />
TREATED_BY_DRUG::[[Oral Contraceptives]]</p>
<h2>References</h2>
<ul>
<li>Rotterdam ESHRE/ASRM-Sponsored PCOS Consensus Workshop Group. (2004). Revised 2003 consensus on diagnostic criteria and long-term health risks related to polycystic ovary syndrome. Fertility and Sterility.</li>
<li>Teede, H. J., et al. (2018). Recommendations from the international evidence-based guideline for the assessment and management of polycystic ovary syndrome.</li>
</ul>

{% endraw %}