---
created: '2026-02-08T19:29:31.589573Z'
description: A progressive neurodegenerative disorder and the most common cause of
  dementia. Characterized by extracellular amyloid-beta plaques and intracellular
  tau neurofibrillary tangles.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/condition/alzheimers-disease/
slug: alzheimers-disease
tags:
- condition
- neurodegeneration
- dementia
- aging
title: Alzheimer's Disease
type: condition
updated: '2026-02-08T19:29:31.589573Z'
---

# Alzheimer's Disease

## Overview
Alzheimer's Disease (AD) is a fatal neurodegenerative condition characterized by the progressive loss of memory and cognitive function. Pathologically, it is defined by the accumulation of **Amyloid-Beta (Aβ)** plaques and **Tau** neurofibrillary tangles, leading to synaptic failure and neuronal death.

## Key Mechanisms
1.  **Amyloid Cascade Hypothesis**: Imbalance between Aβ production (via BACE1/Gamma-secretase) and clearance leads to oligomerization and plaque formation.
2.  **Tau Pathology**: Hyperphosphorylation of Tau causes microtubule disintegration and tangle formation.
3.  **Cholinergic Hypothesis**: Loss of cholinergic neurons in the basal forebrain contributes to cognitive decline.
4.  **Genetic Risk**: APOE4 allele is the strongest genetic risk factor for late-onset AD.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- ← [[Omega-3_DHA]] (supplement)
```yaml
last_modified: '2026-01-26T07:08:39.685849'
source: obsidian
```
- ← [[Mild_Cognitive_Impairment]] (condition)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- ← [[Vascular_Dementia]] (condition)
```yaml
last_modified: '2026-01-26T07:07:38.779185'
source: obsidian
```
- ← [[Donepezil]] (drug)
```yaml
last_modified: '2026-01-26T07:07:41.406812'
source: obsidian
```
- ← [[Memantine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:42.400780'
source: obsidian
```
- ← [[Alpha-Synuclein_SAA]] (examination)
```yaml
last_modified: '2026-01-26T07:07:44.082636'
source: obsidian
```
- ← [[Amyloid_PET_Imaging]] (examination)
```yaml
last_modified: '2026-01-26T07:07:44.235597'
source: obsidian
```
- ← [[EEG]] (examination)
```yaml
last_modified: '2026-01-26T07:07:44.540742'
source: obsidian
```
- ← [[FDG-PET]] (examination)
```yaml
last_modified: '2026-01-26T07:07:44.865410'
source: obsidian
```
- ← [[Lumbar_Puncture]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.028418'
source: obsidian
```
- ← [[MRI_Volumetry]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.199836'
source: obsidian
```
- ← [[Neuropsychological_Testing]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.380650'
source: obsidian
```
- ← [[Retinal_Imaging_for_Neurodegeneration]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.549973'
source: obsidian
```
- ← [[Tau_PET_Imaging]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.698956'
source: obsidian
```
- ← [[ApoE]] (protein)
```yaml
last_modified: '2026-01-26T07:08:28.742616'
source: obsidian
```
- ← [[APP]] (protein)
```yaml
last_modified: '2026-01-26T07:08:28.853140'
source: obsidian
```
- ← [[Cognitive_Training]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:47.835016'
source: obsidian
```
- ← [[Mediterranean_Diet]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:48.486140'
source: obsidian
```
- ← [[Sleep_Optimization]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:48.863953'
source: obsidian
```
- ← [[Amyloid-Beta_4240_Ratio]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:09.882854'
source: obsidian
```
- ← [[BDNF]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:10.191066'
source: obsidian
```
- ← [[Blood_NfL]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:10.597897'
source: obsidian
```
- ← [[Digital_Biomarkers_for_Neurodegeneration]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:11.464163'
source: obsidian
```
- ← [[GFAP]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:12.668732'
source: obsidian
```
- ← [[MTBR-tau243]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:14.924796'
source: obsidian
```
- ← [[Neurofilament_Light_Chain]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:15.198842'
source: obsidian
```
- ← [[Neuron-Derived_Exosome_Biomarkers]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:15.353378'
source: obsidian
```
- ← [[Phospho-Tau]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:15.718126'
source: obsidian
```
- ← [[Plasma_Amyloid-Beta_Ratio]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:15.960616'
source: obsidian
```
- ← [[Plasma_p-tau217]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:16.234462'
source: obsidian
```
- ← [[S100B]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:16.720220'
source: obsidian
```
- ← [[Total_Tau]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:17.683830'
source: obsidian
```
- ← [[Amyloid_Cascade]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:11.155642'
source: obsidian
```
- ← [[Autophagy-Lysosomal_Pathway]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:11.341134'
source: obsidian
```
- ← [[Neuroinflammation]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:12.428247'
source: obsidian
```
- ← [[Tau_Phosphorylation]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:12.950584'
source: obsidian
```
- ← [[BACE1]] (protein)
```yaml
last_modified: '2026-01-26T07:08:29.251699'
source: obsidian
```
- ← [[Presenilin-1]] (protein)
```yaml
last_modified: '2026-01-26T07:08:31.896803'
source: obsidian
```
- ← [[Tau]] (protein)
```yaml
last_modified: '2026-01-26T07:08:34.120727'
source: obsidian
```
- → [[Donepezil]] (drug)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Alzheimers_Disease]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Sleep_Optimization]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Phospho-Tau]] (biomarker)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Autophagy-Lysosomal_Pathway]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Amyloid_PET_Imaging]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[BDNF]] (biomarker)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Blood_NfL]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Neuroinflammation]] (pathway)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[MTBR-tau243]] (biomarker)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Alpha-Synuclein_SAA]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Vascular_Dementia]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Cognitive_Training]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Tau_Phosphorylation]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[GFAP]] (biomarker)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Neuron-Derived_Exosome_Biomarkers]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Amyloid_Cascade]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Amyloid-Beta_4240_Ratio]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Digital_Biomarkers_for_Neurodegeneration]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Lumbar_Puncture]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Plasma_Amyloid-Beta_Ratio]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Omega-3_DHA]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Plasma_p-tau217]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[ApoE]] (protein)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[S100B]] (biomarker)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Tau_PET_Imaging]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Retinal_Imaging_for_Neurodegeneration]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[FDG-PET]] (examination)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[EEG]] (examination)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Neuropsychological_Testing]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Neurofilament_Light_Chain]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[MRI_Volumetry]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Total_Tau]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Mediterranean_Diet]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Mild_Cognitive_Impairment]] (entity)
```yaml
last_modified: '2026-01-26T06:47:30.206636'
source: obsidian
```
- → [[Aducanumab]] (entity)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Presenilin-1]] (protein)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[ApoE4]] (entity)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Dementia]] (entity)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Lecanemab]] (entity)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[BACE1]] (protein)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Tau]] (protein)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Memantine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[APP]] (protein)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Mild_Cognitive_Impairment]] (condition)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- → [[Alzheimers_Disease]] (condition)
```yaml
last_modified: '2026-01-26T06:03:22.861503'
source: obsidian
```

### Increases Risk For
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships

### Associated Proteins
- → [[APP]] (protein) - *Precursor of Amyloid Beta*
- → [[Tau]] (protein) - *Forms Neurofibrillary Tangles*
- → [[ApoE4]] (protein) - *Major Genetic Risk Factor*
- → [[BACE1]] (protein) - *Rate-limiting enzyme for Aβ production*
- → [[Presenilin-1]] (protein) - *Part of Gamma-secretase*

### Treatments
- ← [[Donepezil]] (drug) - *Cholinesterase Inhibitor*
- ← [[Memantine]] (drug) - *NMDA Antagonist*
- ← [[Aducanumab]] (drug) - *Anti-Amyloid Antibody*
- ← [[Lecanemab]] (drug) - *Anti-Amyloid Antibody*

### Related Conditions
- → [[Dementia]] (condition)
- → [[Mild Cognitive Impairment]] (condition)

## References
1. **Review**: Scheltens, P., et al. (2021). "Alzheimer's disease." *The Lancet*.