---
created: '2026-02-08T19:25:12.933093Z'
description: A hormonal disorder caused by prolonged exposure to high levels of cortisol,
  resulting in a variety of physical and metabolic changes.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/Condition/cushings-syndrome/
slug: cushings-syndrome
tags:
- Condition
- condition
- endocrinology
- hormones
- metabolic
- function-health
title: Cushings Syndrome
type: Condition
updated: '2026-02-08T19:25:12.933093Z'
---

# Cushings Syndrome

## Overview

Cushing's Syndrome (Hypercortisolism) can be **Exogenous** (caused by medications) or **Endogenous** (caused by the body). **Cushing's Disease** specifically refers to the condition when a pituitary tumor overproduces ACTH. Excess cortisol has devastating effects on metabolism, skin integrity, and bone density.

## Complications

- **Osteoporosis and Fractures.**
- **Type 2 Diabetes.**
- **Hypertension.**
- **Muscle Wasting.**
- **Immune Suppression.**

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[Cushings_Syndrome]] (entity)
```yaml
last_modified: '2026-01-26T06:47:36.722958'
source: obsidian
```
- → [[Cushings_Syndrome]] (condition)
```yaml
last_modified: '2026-01-26T06:03:33.576234'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships

ASSOCIATED_WITH_CONDITION::[[Adrenal Disease]]
ASSOCIATED_WITH_CONDITION::[[Pituitary Disorder]]
ASSOCIATED_WITH_CONDITION::[[Osteoporosis]]
ASSOCIATED_WITH_CONDITION::[[Diabetes Mellitus]]
AFFECTS_ORGAN::[[Adrenal Glands]]
AFFECTS_ORGAN::[[Pituitary Gland]]
ASSOCIATED_WITH_BIOMARKER::[[Cortisol]] - Characteristically high.
ASSOCIATED_WITH_BIOMARKER::[[ACTH]]
ASSOCIATED_WITH_BIOMARKER::[[HbA1c]]
TREATED_BY_INTERVENTION::[[Surgery]] - Tumor removal.

## References

- Lacroix, A., et al. (2015). Cushing's syndrome. The Lancet.
- Mayo Clinic. Cushing syndrome.
- Endocrine Society. Cushing's Syndrome Clinical Practice Guidelines.