---
created: '2026-02-08T19:45:44.467366Z'
description: Chronic inflammation is a prolonged inflammatory response that can last
  for weeks, months, or even years. It is characterized by the simultaneous active
  inflammation, tissue destruction, and attempts at healing.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/condition/chronic-inflammation/
slug: chronic-inflammation
tags:
- condition
- inflammation
- chronic
title: Chronic Inflammation
type: condition
updated: '2026-02-08T19:45:44.467366Z'
---

# Chronic Inflammation

## Overview
Chronic inflammation is a prolonged inflammatory response that can last for weeks, months, or even years. It is characterized by the simultaneous active inflammation, tissue destruction, and attempts at healing.

## Clinical Presentation
### Symptoms
Not specified.



## Diagnostic Information


## Risk Factors



## Management


### Interventions
```dataview
TABLE 
    effectiveness AS "Effectiveness",
    confidence_score AS "Confidence"
FROM [[Chronic Inflammation]] AND #intervention
SORT confidence_score DESC
```





## Other Relationships
### Complicates
- ← [[Diabetes]] (condition)
```yaml
last_modified: '2025-12-28T11:01:07.546536'
source: obsidian
```

### Related To
- ← [[Ashwagandha]] (drug)
```yaml
last_modified: '2025-12-28T11:01:08.042376'
source: obsidian
```

### Reduces Condition
- ← [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2026-01-19T09:31:54.373077'
source: obsidian
```
- ← [[Turmeric]] (drug)
```yaml
last_modified: '2025-12-28T11:01:09.383582'
source: obsidian
```
- ← [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:10.499868'
source: obsidian
```
- ← [[Magnesium]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:22.460852'
source: obsidian
```

### Improves Condition
- ← [[Creatine]] (drug)
```yaml
last_modified: '2025-12-28T11:01:08.561961'
source: obsidian
```
- ← [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:24.349991'
source: obsidian
```

### May Improve Condition
- ← [[Curcumin]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:20.513724'
source: obsidian
```
- ← [[Vitamin_D]] (biomarker)
```yaml
last_modified: '2025-12-28T11:01:28.052889'
source: obsidian
```

### Worsened By Condition
- → [[Diabetes]] (condition)
```yaml
last_modified: '2025-12-28T11:01:07.100000'
source: obsidian
```

### Reduced By Drug
- → [[Turmeric]] (drug)
```yaml
last_modified: '2025-12-28T11:01:07.143561'
source: obsidian
```
- → [[Ashwagandha]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:07.188023'
source: obsidian
```
- → [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:07.226923'
source: obsidian
```

### Reduced By Intervention
- → [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:07.263748'
source: obsidian
```

### Related
- → [[Ashwagandha]] (drug)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Coenzyme_Q10]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Red_Light_Therapy]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Omega-3_Fatty_Acids]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Vitamin_D]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[basophils]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[eosinophils]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[lymphocytes]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[monocytes]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[neutrophils]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[white_blood_cell_count]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[iron]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[iron_saturation]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[iron_binding_capacity]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[omega_3_total]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[omega_6_omega_3_ratio]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[omega_6_total]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[omega_6_arachidonic_acid]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[omega_6_linoleic_acid]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[platelet_count]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[red_cell_distribution_width_rdw]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[protein_urine]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[hs-CRP]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[ESR]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Fibrinogen]] (biomarker)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Homocysteine]] (biomarker)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Cardiovascular_Disease]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Alzheimers_Disease]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Cancer]] (condition)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Fish_Oil]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Cardiovascular_Risk_Pattern]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Kidney_Stress_Pattern]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[NF-kB_Pathway]] (entity)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Ashwagandha]] (supplement)
```yaml
last_modified: '2025-12-27T18:02:21.041493'
source: obsidian
```
- → [[Turmeric]] (drug)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Diabetes]] (condition)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Curcumin]] (supplement)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Creatine]] (drug)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```
- → [[Magnesium]] (supplement)
```yaml
last_modified: '2026-01-19T09:30:57.049485'
source: obsidian
```

### May Detect
- ← [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[basophils]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[eosinophils]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[lymphocytes]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[monocytes]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[neutrophils]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[white_blood_cell_count]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[iron]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[iron_saturation]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[iron_binding_capacity]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_3_total]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_omega_3_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_total]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_arachidonic_acid]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_linoleic_acid]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[platelet_count]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[red_cell_distribution_width_rdw]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[protein_urine]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```

### Requires Biomarker
- → [[hs-CRP]] (entity)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- → [[ESR]] (entity)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- → [[Fibrinogen]] (biomarker)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- → [[Homocysteine]] (biomarker)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```

### Increases Risk For
- → [[Cardiovascular_Disease]] (entity)
```yaml
last_modified: '2026-01-19T08:39:23.827674'
source: obsidian
```
- → [[Alzheimers_Disease]] (entity)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- → [[Cancer]] (entity)
```yaml
last_modified: '2026-01-19T08:39:23.827674'
source: obsidian
```
- → [[Cardiovascular_Disease]] (condition)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- → [[Cancer]] (condition)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```

### Improved By Supplement
- → [[Fish_Oil]] (entity)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- → [[Curcumin]] (supplement)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- → [[Vitamin_D]] (biomarker)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```

### Related Pattern
- ← [[Cardiovascular_Risk_Pattern]] (clinicalpattern)
```yaml
last_modified: '2026-01-19T09:30:36.399745'
source: obsidian
```
- ← [[Kidney_Stress_Pattern]] (clinicalpattern)
```yaml
last_modified: '2026-01-19T09:30:40.032727'
source: obsidian
```

### Modulated By Pathway
- → [[NF-kB_Pathway]] (pathway)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```

## Research
### Recent Studies
```dataview
LIST
FROM [[Chronic Inflammation]] AND #research
WHERE date >= date(today) - dur(1 year)
SORT date DESC
```

### Clinical Trials
```dataview
LIST status
FROM [[Chronic Inflammation]] AND #clinical-trial
SORT date DESC
```

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- ← [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:50.356432'
source: obsidian
```
- ← [[white_blood_cell_count]] (labtest)
```yaml
last_modified: '2026-01-26T07:08:01.704457'
source: obsidian
```
- ← [[red_cell_distribution_width_rdw]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:59.426098'
source: obsidian
```
- ← [[protein_urine]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:59.118633'
source: obsidian
```
- ← [[platelet_count]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:58.738195'
source: obsidian
```
- ← [[omega_6_total]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:58.313774'
source: obsidian
```
- ← [[omega_6_omega_3_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:58.222853'
source: obsidian
```
- ← [[omega_6_linoleic_acid]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:58.130579'
source: obsidian
```
- ← [[omega_6_arachidonic_acid]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:58.057936'
source: obsidian
```
- ← [[omega_3_total]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:57.988761'
source: obsidian
```
- ← [[neutrophils]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:57.605433'
source: obsidian
```
- ← [[monocytes]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:57.436126'
source: obsidian
```
- ← [[lymphocytes]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:56.658204'
source: obsidian
```
- ← [[iron]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:54.949570'
source: obsidian
```
- ← [[iron_saturation]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:54.849989'
source: obsidian
```
- ← [[iron_binding_capacity]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:54.670159'
source: obsidian
```
- ← [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:54.105921'
source: obsidian
```
- ← [[eosinophils]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:52.578220'
source: obsidian
```
- ← [[basophils]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:50.601190'
source: obsidian
```
- → [[Ashwagandha]] (supplement)
```yaml
last_modified: '2025-12-27T18:02:21.041493'
source: obsidian
```
- → [[ESR]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Coenzyme_Q10]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[NF-kB_Pathway]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Cardiovascular_Disease]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Vitamin_D]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Cardiovascular_Risk_Pattern]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Red_Light_Therapy]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Fish_Oil]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[hs-CRP]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Alzheimers_Disease]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Omega-3_Fatty_Acids]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Kidney_Stress_Pattern]] (entity)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[iron_saturation]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[white_blood_cell_count]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[red_cell_distribution_width_rdw]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[monocytes]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[eosinophils]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[omega_6_linoleic_acid]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[iron]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Cancer]] (condition)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[omega_6_omega_3_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Fibrinogen]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[protein_urine]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[platelet_count]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[lymphocytes]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[neutrophils]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[iron_binding_capacity]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[omega_3_total]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Homocysteine]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[omega_6_total]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[omega_6_arachidonic_acid]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[basophils]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Turmeric]] (drug)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Diabetes]] (condition)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Curcumin]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Creatine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Magnesium]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Ashwagandha]] (drug)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- → [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```

### Worsened By Condition
- → [[Diabetes]] (condition)
```yaml
last_modified: '2025-12-28T11:01:07.100000'
source: obsidian
```

### Reduced By Drug
- → [[Turmeric]] (drug)
```yaml
last_modified: '2025-12-28T11:01:07.143561'
source: obsidian
```
- → [[Ashwagandha]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:07.188023'
source: obsidian
```
- → [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:07.226923'
source: obsidian
```

### Reduced By Intervention
- → [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:07.263748'
source: obsidian
```

### Complicates
- ← [[Diabetes]] (condition)
```yaml
last_modified: '2025-12-28T11:01:07.546536'
source: obsidian
```

### Improves Condition
- ← [[Creatine]] (drug)
```yaml
last_modified: '2025-12-28T11:01:08.561961'
source: obsidian
```
- ← [[Omega-3_Fatty_Acids]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:24.349991'
source: obsidian
```

### Related To
- ← [[Ashwagandha]] (drug)
```yaml
last_modified: '2025-12-28T11:01:08.042376'
source: obsidian
```

### Reduces Condition
- ← [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2026-01-19T12:00:43.836954'
source: obsidian
```
- ← [[Turmeric]] (drug)
```yaml
last_modified: '2025-12-28T11:01:09.383582'
source: obsidian
```
- ← [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:10.499868'
source: obsidian
```
- ← [[Magnesium]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:22.460852'
source: obsidian
```

### May Improve Condition
- ← [[Curcumin]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:20.513724'
source: obsidian
```
- ← [[Vitamin_D]] (biomarker)
```yaml
last_modified: '2025-12-28T11:01:28.052889'
source: obsidian
```

### Requires Biomarker
- → [[Homocysteine]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- → [[ESR]] (entity)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- → [[hs-CRP]] (entity)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- → [[Fibrinogen]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```

### Increases Risk For
- → [[Cardiovascular_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- → [[Alzheimers_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- → [[Cancer]] (condition)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```

### Improved By Supplement
- → [[Vitamin_D]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- → [[Fish_Oil]] (entity)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- → [[Curcumin]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```

### Related Pattern
- ← [[Cardiovascular_Risk_Pattern]] (clinicalpattern)
```yaml
last_modified: '2026-01-26T07:07:20.311340'
source: obsidian
```
- ← [[Kidney_Stress_Pattern]] (clinicalpattern)
```yaml
last_modified: '2026-01-26T07:07:21.334944'
source: obsidian
```

### Modulated By Pathway
- → [[NF-kB_Pathway]] (pathway)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```

### May Detect
- ← [[iron_saturation]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[protein_urine]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[red_cell_distribution_width_rdw]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[platelet_count]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_linoleic_acid]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_arachidonic_acid]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_total]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_6_omega_3_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[omega_3_total]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[iron_binding_capacity]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[basophils]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[eosinophils]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[lymphocytes]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[monocytes]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[neutrophils]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[white_blood_cell_count]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[iron]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
<!-- NEO4J_CONTENT_END -->
## References
No references available.