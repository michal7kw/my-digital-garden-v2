---
created: '2026-02-08T19:45:44.488427Z'
description: An intermediate stage between normal age-related cognitive decline and
  dementia. Represents increased risk for progression to Alzheimer's disease or other
  dementias, but may remain stable or improve.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/condition/mild-cognitive-impairment/
slug: mild-cognitive-impairment
tags:
- condition
- neurodegeneration
- cognition
- aging
- prodromal
title: Mild Cognitive Impairment
type: condition
updated: '2026-02-08T19:45:44.488427Z'
---

# Mild Cognitive Impairment

## Overview
Mild Cognitive Impairment (MCI) is a clinical syndrome characterized by cognitive decline greater than expected for age but not severe enough to meet criteria for dementia. It represents a critical window for early intervention and prevention strategies.

## Subtypes
1. **Amnestic MCI (aMCI)**: Primarily memory impairment; higher risk of progression to Alzheimer's disease
2. **Non-Amnestic MCI**: Impairment in other cognitive domains (attention, language, visuospatial); may progress to other dementias

## Diagnostic Criteria (NIA-AA)
1. Concern about cognitive change from patient, informant, or clinician
2. Objective evidence of impairment in one or more cognitive domains
3. Preservation of independence in functional abilities
4. Not demented

## Progression Rates
- **10-15%** annual conversion to dementia (vs. 1-2% in normal aging)
- **~30-40%** remain stable over 5 years
- **~15-20%** may revert to normal cognition

## Risk Factors
- Age (primary risk factor)
- APOE ε4 allele
- Cardiovascular risk factors (hypertension, diabetes, dyslipidemia)
- Low education/cognitive reserve
- Depression
- Sleep disorders

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- ← [[Amyloid_PET_Imaging]] (examination)
```yaml
last_modified: '2026-01-26T07:07:44.235597'
source: obsidian
```
- ← [[FDG-PET]] (examination)
```yaml
last_modified: '2026-01-26T07:07:44.865410'
source: obsidian
```
- ← [[Lumbar_Puncture]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.028418'
source: obsidian
```
- ← [[MRI_Volumetry]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.199836'
source: obsidian
```
- ← [[Neuropsychological_Testing]] (examination)
```yaml
last_modified: '2026-01-26T07:07:45.380650'
source: obsidian
```
- ← [[Cognitive_Training]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:47.835016'
source: obsidian
```
- ← [[Mediterranean_Diet]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:48.486140'
source: obsidian
```
- ← [[Sleep_Optimization]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:48.863953'
source: obsidian
```
- ← [[Amyloid-Beta_4240_Ratio]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:09.882854'
source: obsidian
```
- ← [[BDNF]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:10.191066'
source: obsidian
```
- ← [[Phospho-Tau]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:15.718126'
source: obsidian
```
- ← [[Plasma_Amyloid-Beta_Ratio]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:15.960616'
source: obsidian
```
- ← [[Plasma_p-tau217]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:16.234462'
source: obsidian
```
- ← [[Alzheimers_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:23.210589'
source: obsidian
```
- ← [[Amyloid_Cascade]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:11.155642'
source: obsidian
```
- ← [[Bacopa_Monnieri]] (supplement)
```yaml
last_modified: '2026-01-26T07:08:35.750324'
source: obsidian
```
- ← [[Omega-3_DHA]] (supplement)
```yaml
last_modified: '2026-01-26T07:08:39.685849'
source: obsidian
```
- → [[Neurofilament_Light_Chain]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Mild_Cognitive_Impairment]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Lewy_Body_Dementia]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Mediterranean_Diet]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[BDNF]] (biomarker)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Sleep_Optimization]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Vascular_Dementia]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Phospho-Tau]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Phosphatidylserine]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Alzheimers_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Neurofilament_Light_Chain]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Amyloid-Beta_4240_Ratio]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Total_Tau]] (biomarker)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Cognitive_Training]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Sleep_Optimization]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Exercise]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[FDG-PET]] (examination)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Mediterranean_Diet]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Vascular_Dementia]] (condition)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Lions_Mane]] (entity)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Omega-3_DHA]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Lewy_Body_Dementia]] (entity)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Bacopa_Monnieri]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Hippocampal_Volume_MRI]] (entity)
```yaml
last_modified: '2026-01-26T07:07:33.408670'
source: obsidian
```
- → [[Lumbar_Puncture]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[MRI_Volumetry]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Amyloid_Cascade]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Plasma_Amyloid-Beta_Ratio]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Amyloid_PET_Imaging]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Omega-3_DHA]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Plasma_p-tau217]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Hippocampal_Volume_MRI]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Bacopa_Monnieri]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Cognitive_Training]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Total_Tau]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Alzheimers_Disease]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Neuropsychological_Testing]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Amyloid-Beta_4240_Ratio]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Lions_Mane]] (entity)
```yaml
last_modified: '2026-01-26T06:47:46.716462'
source: obsidian
```
- → [[Mild_Cognitive_Impairment]] (condition)
```yaml
last_modified: '2026-01-26T06:03:48.937777'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships

### Biomarkers for Progression Risk
- ← [[Amyloid-Beta 42/40 Ratio]] (biomarker) - *CSF or PET; indicates amyloid pathology*
- ← [[Phospho-Tau]] (biomarker) - *Indicates tau pathology and higher conversion risk*
- ← [[Total Tau]] (biomarker) - *Marker of neurodegeneration*
- ← [[Neurofilament Light Chain]] (biomarker) - *Predicts faster decline*
- ← [[Hippocampal Volume MRI]] (examination) - *Structural neuroimaging*
- ← [[FDG-PET]] (examination) - *Metabolic imaging*

### Progression Conditions
- → [[Alzheimer's Disease]] (condition) - *Most common progression path*
- → [[Vascular Dementia]] (condition) - *Alternative progression*
- → [[Lewy Body Dementia]] (condition) - *Alternative progression*

### Preventive Interventions
- ← [[Cognitive Training]] (intervention) - *May slow progression*
- ← [[Exercise]] (intervention) - *Cardiovascular and neuroprotective*
- ← [[Mediterranean Diet]] (intervention) - *Dietary pattern*
- ← [[Sleep Optimization]] (intervention) - *Glymphatic clearance*

### Neuroprotective Supplements
- ← [[Omega-3 DHA]] (supplement) - *Brain structure support*
- ← [[Bacopa Monnieri]] (supplement) - *Memory enhancement*
- ← [[Lion's Mane]] (supplement) - *NGF stimulation*
- ← [[Phosphatidylserine]] (supplement) - *Membrane support*

## References
1. **Criteria**: Albert, M.S., et al. (2011). "The diagnosis of mild cognitive impairment." *Alzheimer's & Dementia*.
2. **Biomarkers**: Jack, C.R., et al. (2018). "NIA-AA Research Framework." *Alzheimer's & Dementia*.