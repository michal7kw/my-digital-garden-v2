---
created: '2026-02-08T19:45:44.465324Z'
description: A broad group of disorders of the heart and blood vessels, often caused
  by atherosclerosis and leading to events such as heart attacks and strokes.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/condition/cardiovascular-disease/
slug: cardiovascular-disease
tags:
- condition
- cardiology
- vascular
- chronic
- function-health
title: Cardiovascular Disease
type: condition
updated: '2026-02-08T19:45:44.465324Z'
---

# Cardiovascular Disease

## Overview
A broad group of disorders of the heart and blood vessels, often caused by atherosclerosis and leading to events such as heart attacks and strokes.

## Clinical Presentation
### Symptoms
Not specified.



## Diagnostic Information


## Risk Factors



## Management


### Interventions
```dataview
TABLE 
    effectiveness AS "Effectiveness",
    confidence_score AS "Confidence"
FROM [[Cardiovascular Disease]] AND #intervention
SORT confidence_score DESC
```





## Other Relationships
### May Detect
- ← [[apolipoprotein_b_apob]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[hdl_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[hdl_large]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_medium]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_particle_number]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_pattern]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_peak_size]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_small]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[non_hdl_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[total_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[total_cholesterol_hdl_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[triglycerides]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[mean_platelet_volume_mpv]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[uric_acid]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```

### Increases Risk For
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-19T09:30:37.999694'
source: obsidian
```
- ← [[Kidney_Stress_Pattern]] (clinicalpattern)
```yaml
last_modified: '2026-01-19T09:30:40.032727'
source: obsidian
```
- ← [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2026-01-19T09:30:42.706915'
source: obsidian
```
- ← [[thyroid_dysfunction]] (condition)
```yaml
last_modified: '2026-01-19T09:30:43.393690'
source: obsidian
```

### Related
- → [[Cardiovascular_Disease]] (condition)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[apolipoprotein_b_apob]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[hdl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[hdl_large]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[ldl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[ldl_medium]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[ldl_particle_number]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[ldl_pattern]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[ldl_peak_size]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[ldl_small]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[non_hdl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[total_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[total_cholesterol_hdl_ratio]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[triglycerides]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[mean_platelet_volume_mpv]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[uric_acid]] (labtest)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[Kidney_Stress_Pattern]] (entity)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[Metabolic_Syndrome]] (entity)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```
- → [[Thyroid_Dysfunction]] (entity)
```yaml
last_modified: '2026-01-19T09:30:52.934046'
source: obsidian
```

## Research
### Recent Studies
```dataview
LIST
FROM [[Cardiovascular Disease]] AND #research
WHERE date >= date(today) - dur(1 year)
SORT date DESC
```

### Clinical Trials
```dataview
LIST status
FROM [[Cardiovascular Disease]] AND #clinical-trial
SORT date DESC
```

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- ← [[ldl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:55.218776'
source: obsidian
```
- ← [[Mediterranean_Diet]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:48.486140'
source: obsidian
```
- ← [[uric_acid]] (labtest)
```yaml
last_modified: '2026-01-26T07:08:01.406769'
source: obsidian
```
- ← [[triglycerides]] (labtest)
```yaml
last_modified: '2026-01-26T07:08:01.151970'
source: obsidian
```
- ← [[total_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:08:00.993626'
source: obsidian
```
- ← [[total_cholesterol_hdl_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:08:00.896437'
source: obsidian
```
- ← [[non_hdl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:57.721942'
source: obsidian
```
- ← [[mean_platelet_volume_mpv]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:57.181105'
source: obsidian
```
- ← [[ldl_small]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:55.657574'
source: obsidian
```
- ← [[ldl_peak_size]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:55.569821'
source: obsidian
```
- ← [[ldl_pattern]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:55.480705'
source: obsidian
```
- ← [[ldl_particle_number]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:55.401389'
source: obsidian
```
- ← [[ldl_medium]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:55.300667'
source: obsidian
```
- ← [[apolipoprotein_b_apob]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:50.100619'
source: obsidian
```
- ← [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:50.356432'
source: obsidian
```
- ← [[hdl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:53.581046'
source: obsidian
```
- ← [[hdl_large]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:53.665896'
source: obsidian
```
- ← [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:54.105921'
source: obsidian
```
- → [[hdl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[Mediterranean_Diet]] (entity)
```yaml
last_modified: '2026-01-26T06:47:33.038221'
source: obsidian
```
- → [[Thyroid_Dysfunction]] (entity)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[Cardiovascular_Disease]] (entity)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[thyroid_dysfunction]] (entity)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[Metabolic_Syndrome]] (entity)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[Kidney_Stress_Pattern]] (entity)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[total_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[mean_platelet_volume_mpv]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[ldl_particle_number]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[ldl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[hdl_large]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[apolipoprotein_b_apob]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[ldl_medium]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[total_cholesterol_hdl_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[ldl_small]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[triglycerides]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[non_hdl_cholesterol]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[ldl_peak_size]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[uric_acid]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[ldl_pattern]] (labtest)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- → [[Thyroid_Dysfunction]] (condition)
```yaml
last_modified: '2026-01-25T19:20:04.835843'
source: obsidian
```
- → [[Cardiovascular_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```

### Increases Risk For
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-26T07:07:20.520330'
source: obsidian
```
- ← [[Kidney_Stress_Pattern]] (clinicalpattern)
```yaml
last_modified: '2026-01-26T07:07:21.334944'
source: obsidian
```
- ← [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2026-01-26T07:07:21.922538'
source: obsidian
```
- ← [[Thyroid_Dysfunction]] (condition)
```yaml
last_modified: '2026-01-26T07:07:22.098625'
source: obsidian
```

### May Detect
- ← [[apolipoprotein_b_apob]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[hdl_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[hdl_large]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[high_sensitivity_c_reactive_protein_hs_crp]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_medium]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_particle_number]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_pattern]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_peak_size]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[ldl_small]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[non_hdl_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[total_cholesterol]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[total_cholesterol_hdl_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[triglycerides]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[arachidonic_acid_epa_ratio]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[mean_platelet_volume_mpv]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- ← [[uric_acid]] (labtest)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
<!-- NEO4J_CONTENT_END -->
## References
No references available.