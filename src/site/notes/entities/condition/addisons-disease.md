---
created: '2026-02-08T19:25:12.917889Z'
description: A rare, chronic disorder where the adrenal glands do not produce enough
  cortisol and often insufficient aldosterone, usually due to autoimmune damage.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/Condition/addisons-disease/
slug: addisons-disease
tags:
- Condition
- condition
- endocrinology
- hormones
- autoimmune
- function-health
title: Addisons Disease
type: Condition
updated: '2026-02-08T19:25:12.917889Z'
---

# Addisons Disease

## Overview

Addison's Disease (Primary Adrenal Insufficiency) occurs when the adrenal cortex is damaged. Most cases in developed nations are **Autoimmune**. Without treatment, it can lead to a life-threatening **Adrenal Crisis**, characterized by shock and organ failure.

## Clinical Indicators

- **Hyperpigmentation:** Due to excess ACTH cross-reacting with melanocyte receptors.
- **Electrolyte Abnormalities:** Low sodium and high potassium.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[Addisons_Disease]] (entity)
```yaml
last_modified: '2026-01-26T06:47:29.446824'
source: obsidian
```
- → [[Addisons_Disease]] (condition)
```yaml
last_modified: '2026-01-26T06:03:21.540212'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships

ASSOCIATED_WITH_CONDITION::[[Adrenal Disease]]
ASSOCIATED_WITH_CONDITION::[[Type 1 Diabetes]]
ASSOCIATED_WITH_CONDITION::[[Hypothyroidism]]
AFFECTS_ORGAN::[[Adrenal Glands]]
ASSOCIATED_WITH_BIOMARKER::[[Cortisol]] - Markedly low.
ASSOCIATED_WITH_BIOMARKER::[[Aldosterone]] - Low.
ASSOCIATED_WITH_BIOMARKER::[[ACTH]] - High.
TREATED_BY_DRUG::[[Hydrocortisone]]
TREATED_BY_DRUG::[[Fludrocortisone]]

## References

- Bornstein, S. R., et al. (2016). Diagnosis and Treatment of Primary Adrenal Insufficiency: An Endocrine Society Clinical Practice Guideline.
- Mayo Clinic. Addison's disease.
- National Adrenal Diseases Foundation. Addison's Disease.