---
created: '2026-02-08T19:29:31.704926Z'
description: Thyroxine (T4) Freeis one of the main hormones made by the thyroid. It
  acts largely as a precursor, since most of it is later converted into T3, the more
  active form. Measuring free T4 shows how much thyroid hormone is being produced
  and available for conversion, providing valuable insight into thyroid function.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/labtest/thyroxine-t4-free/
slug: thyroxine-t4-free
tags:
- labtest
title: thyroxine_t4_free
type: labtest
updated: '2026-02-08T19:29:31.704926Z'
---

# Thyroxine (T4) Free

## Overview
Thyroxine (T4) Freeis one of the main hormones made by the thyroid. It acts largely as a precursor, since most of it is later converted into T3, the more active form. Measuring free T4 shows how much thyroid hormone is being produced and available for conversion, providing valuable insight into thyroid function.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Blood Panel |
| **Subcategory** | Thyroid || **Test Type** | Standard |
| **Sample Type** | Blood |
| **Collection Method** | Venipuncture |
| **Turnaround Time** | 3-7 days || **Price Range** | $null-$null || **HSA Eligible** | ✅ Yes || **FSA Eligible** | ✅ Yes |




## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
Useful for monitoring or diagnosing:

- [[Hypothyroidism]]
- [[Grave'S Disease]]
- [[Hashimoto'S Thyroiditis]]
- [[Thyroid Storm]]
- [[Chronic Illness Or Secondary Thyroid Dysfunction]]

## Reference Ranges

Reference ranges vary by laboratory and testing method. Consult your healthcare provider for interpretation.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[GraveS_Disease]] (entity)
```yaml
last_modified: '2026-01-26T06:48:27.222606'
source: obsidian
```
- → [[Chronic_Illness_Or_Secondary_Thyroid_Dysfunction]] (entity)
```yaml
last_modified: '2026-01-26T06:48:27.222606'
source: obsidian
```
- → [[HashimotoS_Thyroiditis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:27.222606'
source: obsidian
```
- → [[Thyroid_Storm]] (entity)
```yaml
last_modified: '2026-01-26T06:48:27.222606'
source: obsidian
```
- → [[Hypothyroidism]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.714808'
source: obsidian
```
- → [[Chronic_Illness_Or_Secondary_Thyroid_Dysfunction]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.714808'
source: obsidian
```
- → [[HashimotoS_Thyroiditis]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.714808'
source: obsidian
```
- → [[GraveS_Disease]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.714808'
source: obsidian
```
- → [[Thyroid_Storm]] (condition)
```yaml
last_modified: '2026-01-26T07:08:00.714808'
source: obsidian
```
- ← [[extended_thyroid_health]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:03.644052'
source: obsidian
```
- → [[extended_thyroid_health]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:27.222606'
source: obsidian
```
- ← [[Thyroid_Storm]] (condition)
```yaml
last_modified: '2026-01-26T06:47:53.735169'
source: obsidian
```

### May Detect
- → [[Thyroid_Storm]] (condition)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```

### Included In
- → [[extended_thyroid_health]] (labtestpanel)
<!-- NEO4J_CONTENT_END -->
## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:32:42.884256*