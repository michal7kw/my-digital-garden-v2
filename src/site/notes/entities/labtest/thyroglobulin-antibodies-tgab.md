---
created: '2026-02-08T19:45:44.586045Z'
description: Thyroglobulin antibodies are proteins made by the immune system that
  mistakenly target thyroglobulin, a protein the thyroid uses to make hormones. Their
  presence can signal autoimmune activity affecting the thyroid, which may interfere
  with hormone production and metabolism.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/labtest/thyroglobulin-antibodies-tgab/
slug: thyroglobulin-antibodies-tgab
tags:
- labtest
title: thyroglobulin_antibodies_tgab
type: labtest
updated: '2026-02-08T19:45:44.586045Z'
---

# Thyroglobulin Antibodies (TgAb)

## Overview
Thyroglobulin antibodies are proteins made by the immune system that mistakenly target thyroglobulin, a protein the thyroid uses to make hormones. Their presence can signal autoimmune activity affecting the thyroid, which may interfere with hormone production and metabolism.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Blood Panel |
| **Subcategory** | Thyroid || **Test Type** | Standard |
| **Sample Type** | Blood |
| **Collection Method** | Venipuncture |
| **Turnaround Time** | 3-7 days || **Price Range** | $null-$null || **HSA Eligible** | ✅ Yes || **FSA Eligible** | ✅ Yes |




## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
Useful for monitoring or diagnosing:

- [[Hashimoto’S Thyroiditis]]
- [[Autoimmune Hypothyroidism]]
- [[Other Autoimmune Diseases (E.G., Type 1 Diabetes, Rheumatoid Arthritis)]]
- [[Subclinical Hypothyroidism]]
- [[Papillary Thyroid Cancer]]

## Reference Ranges

Reference ranges vary by laboratory and testing method. Consult your healthcare provider for interpretation.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[Subclinical_Hypothyroidism]] (entity)
```yaml
last_modified: '2026-01-26T06:48:26.387865'
source: obsidian
```
- → [[Papillary_Thyroid_Cancer]] (entity)
```yaml
last_modified: '2026-01-26T06:48:26.387865'
source: obsidian
```
- → [[HashimotoS_Thyroiditis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:26.387865'
source: obsidian
```
- → [[Autoimmune_Hypothyroidism]] (entity)
```yaml
last_modified: '2026-01-26T06:48:26.387865'
source: obsidian
```
- → [[Other_Autoimmune_Diseases_EG_Type_1_Diabetes_Rheumatoid_Arthritis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:26.387865'
source: obsidian
```
- → [[Papillary_Thyroid_Cancer]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.175550'
source: obsidian
```
- → [[Autoimmune_Hypothyroidism]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.175550'
source: obsidian
```
- → [[Other_Autoimmune_Diseases_EG_Type_1_Diabetes_Rheumatoid_Arthritis]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.175550'
source: obsidian
```
- → [[HashimotoS_Thyroiditis]] (entity)
```yaml
last_modified: '2026-01-26T07:08:00.175550'
source: obsidian
```
- → [[Subclinical_Hypothyroidism]] (condition)
```yaml
last_modified: '2026-01-26T07:08:00.175550'
source: obsidian
```
- ← [[extended_thyroid_health]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:03.644052'
source: obsidian
```
- → [[extended_thyroid_health]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:26.387865'
source: obsidian
```
- ← [[Subclinical_Hypothyroidism]] (condition)
```yaml
last_modified: '2026-01-26T07:07:37.892532'
source: obsidian
```

### May Detect
- → [[Subclinical_Hypothyroidism]] (condition)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```

### Included In
- → [[extended_thyroid_health]] (labtestpanel)
<!-- NEO4J_CONTENT_END -->
## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:32:41.828604*