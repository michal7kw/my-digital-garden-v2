---
created: '2026-02-08T19:29:31.700949Z'
description: Rheumatoid factor (RF) is an antibody commonly associated with autoimmune
  conditions like rheumatoid arthritis, rheumatoid vasculitis, or Sjögren’s syndrome.
  Higher levels of RF typically point to more severe complications of the autoimmune
  condition like joint damage.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/labtest/rheumatoid-factor-rf/
slug: rheumatoid-factor-rf
tags:
- labtest
title: rheumatoid_factor_rf
type: labtest
updated: '2026-02-08T19:29:31.700949Z'
---

# Rheumatoid Factor (RF)

## Overview
Rheumatoid factor (RF) is an antibody commonly associated with autoimmune conditions like rheumatoid arthritis, rheumatoid vasculitis, or Sjögren’s syndrome. Higher levels of RF typically point to more severe complications of the autoimmune condition like joint damage.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Autoimmune |
| **Subcategory** | Other || **Test Type** | Standard |
| **Sample Type** | Blood |
| **Collection Method** | Venipuncture |
| **Turnaround Time** | 3-7 days || **Price Range** | $null-$null || **HSA Eligible** | ✅ Yes || **FSA Eligible** | ✅ Yes |




## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
Useful for monitoring or diagnosing:

- [[Rheumatoid Arthritis]]
- [[Sjögren’S Syndrome]]
- [[Systemic Lupus Erythematosus (Sle)]]
- [[Chronic Infections (Hepatitis C, Endocarditis, Tuberculosis)]]
- [[Vasculitis (Cryoglobulinemic Vasculitis)]]

## Reference Ranges

Reference ranges vary by laboratory and testing method. Consult your healthcare provider for interpretation.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[SjgrenS_Syndrome]] (entity)
```yaml
last_modified: '2026-01-26T06:48:25.382182'
source: obsidian
```
- → [[Chronic_Infections_Hepatitis_C_Endocarditis_Tuberculosis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:25.382182'
source: obsidian
```
- → [[Systemic_Lupus_Erythematosus_Sle]] (entity)
```yaml
last_modified: '2026-01-26T06:48:25.382182'
source: obsidian
```
- → [[Vasculitis_Cryoglobulinemic_Vasculitis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:25.382182'
source: obsidian
```
- → [[Rheumatoid_Arthritis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:25.382182'
source: obsidian
```
- → [[Rheumatoid_Arthritis]] (entity)
```yaml
last_modified: '2026-01-26T07:07:59.588397'
source: obsidian
```
- → [[Systemic_Lupus_Erythematosus_Sle]] (entity)
```yaml
last_modified: '2026-01-26T07:07:59.588397'
source: obsidian
```
- → [[Vasculitis_Cryoglobulinemic_Vasculitis]] (entity)
```yaml
last_modified: '2026-01-26T07:07:59.588397'
source: obsidian
```
- → [[Chronic_Infections_Hepatitis_C_Endocarditis_Tuberculosis]] (entity)
```yaml
last_modified: '2026-01-26T07:07:59.588397'
source: obsidian
```
- → [[SjgrenS_Syndrome]] (entity)
```yaml
last_modified: '2026-01-26T07:07:59.588397'
source: obsidian
```
- ← [[extended_autoimmunity]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:02.700050'
source: obsidian
```
- ← [[celiac_gluten_intolerance]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:02.325688'
source: obsidian
```
- → [[celiac_gluten_intolerance]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:25.382182'
source: obsidian
```
- → [[extended_autoimmunity]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:25.382182'
source: obsidian
```

### Included In
- → [[extended_autoimmunity]] (labtestpanel)
- → [[celiac_gluten_intolerance]] (labtestpanel)
<!-- NEO4J_CONTENT_END -->
## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:32:40.338765*