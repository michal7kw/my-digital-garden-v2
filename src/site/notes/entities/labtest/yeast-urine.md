---
created: '2026-02-08T19:25:13.052054Z'
description: Tested twice a year Yeast in the urine, most often Candida species, may
  sometimes reflect contamination from the skin or genital area, but persistent presence
  often signals a fungal urinary tract infection (UTI) known as candiduria.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/LabTest/yeast-urine/
slug: yeast-urine
tags:
- LabTest
- labtest
title: yeast_urine
type: LabTest
updated: '2026-02-08T19:25:13.052054Z'
---

# Yeast, Urine

## Overview
Tested twice a year Yeast in the urine, most often Candida species, may sometimes reflect contamination from the skin or genital area, but persistent presence often signals a fungal urinary tract infection (UTI) known as candiduria.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Blood Panel |
| **Subcategory** | Kidney Function || **Test Type** | Standard |
| **Sample Type** | Urine |
| **Collection Method** | Spot Urine |
| **Turnaround Time** | 3-7 days || **Price Range** | $null-$null || **HSA Eligible** | ✅ Yes || **FSA Eligible** | ✅ Yes |




## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
Useful for monitoring or diagnosing:

- [[Yeast Infection (Candidiasis)]]
- [[Urinary Tract Infection (Secondary Yeast Involvement)]]
- [[Immune Deficiency]]
- [[Poor Glucose Control]]
- [[Medication Effect (E.G., Antibiotics Or Steroids)]]

## Reference Ranges

Reference ranges vary by laboratory and testing method. Consult your healthcare provider for interpretation.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[Poor_Glucose_Control]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.758720'
source: obsidian
```
- → [[Immune_Deficiency]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.758720'
source: obsidian
```
- → [[Urinary_Tract_Infection_Secondary_Yeast_Involvement]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.758720'
source: obsidian
```
- → [[Medication_Effect_EG_Antibiotics_Or_Steroids]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.758720'
source: obsidian
```
- → [[Yeast_Infection_Candidiasis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.758720'
source: obsidian
```
- → [[Poor_Glucose_Control]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.922942'
source: obsidian
```
- → [[Immune_Deficiency]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.922942'
source: obsidian
```
- → [[Urinary_Tract_Infection_Secondary_Yeast_Involvement]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.922942'
source: obsidian
```
- → [[Yeast_Infection_Candidiasis]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.922942'
source: obsidian
```
- → [[Medication_Effect_EG_Antibiotics_Or_Steroids]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.922942'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:32:46.162945*