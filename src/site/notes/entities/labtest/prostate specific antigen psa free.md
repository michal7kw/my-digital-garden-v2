---
{"dg-publish":true,"permalink":"/entities/labtest/prostate-specific-antigen-psa-free/","tags":["labtest"]}
---

# Prostate Specific Antigen (PSA) %, Free

## Overview
pros-tate speh-sif-ik an-tih-jen (p-s-a) per-sent free Percent free PSA (prostate-specific antigen) measures the share of PSA in the blood that is not attached to proteins. This percentage is used alongside other information to help assess the likelihood of prostate cancer and to guide decisions about further testing, such as a biopsy. It is calculated by dividing free PSA by total PSA and multiplying by 100%. Lower values can be associated with a higher likelihood of cancer, while higher values are more often linked to non cancerous prostate conditions like enlargement or inflammation.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Hormones |
| **Subcategory** | Sex Hormones || **Test Type** | Standard |
| **Sample Type** | Blood |
| **Collection Method** | Venipuncture |
| **Turnaround Time** | 3-7 days || **Price Range** | $null-$null || **HSA Eligible** | ✅ Yes || **FSA Eligible** | ✅ Yes |




## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
General health screening.

## Reference Ranges

Reference ranges vary by laboratory and testing method. Consult your healthcare provider for interpretation.

## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:32:39.204412*