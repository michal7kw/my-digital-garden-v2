---
created: '2026-02-08T19:25:13.048573Z'
description: try-eye-oh-doh-thigh-ruh-neen tee-three free Triiodothyronine (T3) Free
  is the active thyroid hormone that directly regulates metabolism, energy levels,
  and body temperature. Most T3 is made by converting T4 into this active form. Measuring
  free T3 helps show how well this conversion is working and how much active thyroid
  hormone is available for the body to use.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/LabTest/triiodothyronine-t3-free/
slug: triiodothyronine-t3-free
tags:
- LabTest
- labtest
title: triiodothyronine_t3_free
type: LabTest
updated: '2026-02-08T19:25:13.048573Z'
---

# Triiodothyronine (T3) Free

## Overview
try-eye-oh-doh-thigh-ruh-neen tee-three free Triiodothyronine (T3) Free is the active thyroid hormone that directly regulates metabolism, energy levels, and body temperature. Most T3 is made by converting T4 into this active form. Measuring free T3 helps show how well this conversion is working and how much active thyroid hormone is available for the body to use.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Blood Panel |
| **Subcategory** | Thyroid || **Test Type** | Standard |
| **Sample Type** | Blood |
| **Collection Method** | Venipuncture |
| **Turnaround Time** | 3-7 days || **Price Range** | $null-$null || **HSA Eligible** | ✅ Yes || **FSA Eligible** | ✅ Yes |




## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
Useful for monitoring or diagnosing:

- [[Grave'S Disease]]
- [[Hypothyroidism]]
- [[Thyroiditis]]
- [[Euthyroid Sick Syndrome (Low T3 Syndrome In Critical Illness)]]
- [[Pituitary Or Hypothalamic Dysfunction (Central Hypothyroidism)]]

## Reference Ranges

Reference ranges vary by laboratory and testing method. Consult your healthcare provider for interpretation.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[GraveS_Disease]] (entity)
```yaml
last_modified: '2026-01-26T06:48:28.446243'
source: obsidian
```
- → [[Euthyroid_Sick_Syndrome_Low_T3_Syndrome_In_Critical_Illness]] (entity)
```yaml
last_modified: '2026-01-26T06:48:28.446243'
source: obsidian
```
- → [[Pituitary_Or_Hypothalamic_Dysfunction_Central_Hypothyroidism]] (entity)
```yaml
last_modified: '2026-01-26T06:48:28.446243'
source: obsidian
```
- → [[Pituitary_Or_Hypothalamic_Dysfunction_Central_Hypothyroidism]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.245071'
source: obsidian
```
- → [[GraveS_Disease]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.245071'
source: obsidian
```
- → [[Euthyroid_Sick_Syndrome_Low_T3_Syndrome_In_Critical_Illness]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.245071'
source: obsidian
```
- → [[Hypothyroidism]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.245071'
source: obsidian
```
- → [[Thyroiditis]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.245071'
source: obsidian
```
- ← [[extended_thyroid_health]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:03.644052'
source: obsidian
```
- → [[extended_thyroid_health]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:28.446243'
source: obsidian
```

### Included In
- → [[extended_thyroid_health]] (labtestpanel)
<!-- NEO4J_CONTENT_END -->
## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:32:44.203747*