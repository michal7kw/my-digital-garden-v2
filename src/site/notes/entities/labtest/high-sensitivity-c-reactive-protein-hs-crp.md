---
created: '2026-02-08T19:45:44.560942Z'
description: high-sen-sih-tiv see-ree-ak-tiv pro-teen (h-s c-r-p) Tested twice a year
  High-sensitivity C-reactive protein (hs-CRP) can reveal even small amounts of inflammation
  in the body, making it a powerful tool for understanding risk for diseases linked
  to long-term inflammation. Chronic inflammation is connected to nearly every major
  health problem—including heart disease (even before there are symptoms), type 2
  diabetes, high blood pressure, Alzheimer’s, depression, autoimmune conditions, and
  severe allergies like asthma. Elevated hs-CRP levels can signal higher risk for
  these issues, while normal results suggest lower risk, though the test does not
  show the exact cause of the inflammation.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/labtest/high-sensitivity-c-reactive-protein-hs-crp/
slug: high-sensitivity-c-reactive-protein-hs-crp
tags:
- labtest
title: high_sensitivity_c_reactive_protein_hs_crp
type: labtest
updated: '2026-02-08T19:45:44.560942Z'
---

# High-Sensitivity C-Reactive Protein (hs-CRP)

## Overview
high-sen-sih-tiv see-ree-ak-tiv pro-teen (h-s c-r-p) Tested twice a year High-sensitivity C-reactive protein (hs-CRP) can reveal even small amounts of inflammation in the body, making it a powerful tool for understanding risk for diseases linked to long-term inflammation. Chronic inflammation is connected to nearly every major health problem—including heart disease (even before there are symptoms), type 2 diabetes, high blood pressure, Alzheimer’s, depression, autoimmune conditions, and severe allergies like asthma. Elevated hs-CRP levels can signal higher risk for these issues, while normal results suggest lower risk, though the test does not show the exact cause of the inflammation.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Blood Panel |
| **Subcategory** | Lipid Panel || **Test Type** | Standard |
| **Sample Type** | Blood |
| **Collection Method** | Venipuncture |
| **Turnaround Time** | 3-7 days || **Price Range** | $null-$null || **HSA Eligible** | ✅ Yes || **FSA Eligible** | ✅ Yes |




## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
Useful for monitoring or diagnosing:

- [[Chronic Inflammation]]
- [[Cardiovascular Disease]]
- [[Metabolic Syndrome]]
- [[Infection]]
- [[Autoimmune Disease]]

## Reference Ranges

Reference ranges vary by laboratory and testing method. Consult your healthcare provider for interpretation.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[Hs_Crp]] (entity)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- → [[Metabolic_Syndrome]] (entity)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- → [[Chronic_Inflammation]] (entity)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- → [[Autoimmune_Disease]] (entity)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- → [[Cardiovascular_Disease]] (entity)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- ← [[Hs_Crp]] (biomarker)
```yaml
last_modified: '2026-01-26T06:47:19.365461'
source: obsidian
```
- → [[Infection]] (entity)
```yaml
last_modified: '2026-01-26T07:07:54.105921'
source: obsidian
```
- → [[Cardiovascular_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:54.105921'
source: obsidian
```
- → [[Autoimmune_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:54.105921'
source: obsidian
```
- → [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-26T07:07:54.105921'
source: obsidian
```
- → [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2026-01-26T07:07:54.105921'
source: obsidian
```
- ← [[genetic_heart_risks]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:04.533403'
source: obsidian
```
- ← [[extended_immune_regulation]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:03.434318'
source: obsidian
```
- ← [[extended_heart_metabolic]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:02.868961'
source: obsidian
```
- ← [[chronic_inflammatory_response_mold_response]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:02.490694'
source: obsidian
```
- → [[extended_heart_metabolic]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- → [[extended_immune_regulation]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- → [[chronic_inflammatory_response_mold_response]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- → [[genetic_heart_risks]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:15.092143'
source: obsidian
```
- ← [[Metabolic_Syndrome]] (condition)
```yaml
last_modified: '2026-01-26T07:07:32.678592'
source: obsidian
```
- ← [[Chronic_Inflammation]] (condition)
```yaml
last_modified: '2026-01-26T07:07:25.519524'
source: obsidian
```
- ← [[Cardiovascular_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:24.806764'
source: obsidian
```
- ← [[Autoimmune_Disease]] (condition)
```yaml
last_modified: '2026-01-26T07:07:23.757663'
source: obsidian
```

### May Detect
- → [[Autoimmune_Disease]] (condition)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- → [[Metabolic_Syndrome]] (condition)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- → [[Cardiovascular_Disease]] (condition)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```
- → [[Chronic_Inflammation]] (condition)
```yaml
confidence: suggested
created_at: '2026-01-18T16:02:36.677740'
source: function-health
```

### Included In
- → [[extended_immune_regulation]] (labtestpanel)
- → [[chronic_inflammatory_response_mold_response]] (labtestpanel)
- → [[genetic_heart_risks]] (labtestpanel)
- → [[extended_heart_metabolic]] (labtestpanel)
<!-- NEO4J_CONTENT_END -->
## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:32:24.457186*