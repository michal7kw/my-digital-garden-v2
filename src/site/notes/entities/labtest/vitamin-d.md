---
created: '2026-02-08T19:45:44.592132Z'
description: Vitamin D is an essential nutrient for immune health, strong bones and
  muscles, and optimal brain performance. Unlike most vitamins, it isn't widely available
  in food and is mainly made in the body through sun exposure. Because of this, low
  levels are common, and maintaining healthy vitamin D is important to support energy,
  mood, and long-term immune, bone, and cardiovascular health.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/labtest/vitamin-d/
slug: vitamin-d
tags:
- labtest
- nutrient
- vitamin
- immune
- bone
title: vitamin_d
type: labtest
updated: '2026-02-08T19:45:44.592132Z'
---

# Vitamin D

## Overview
Vitamin D is an essential nutrient for immune health, strong bones and muscles, and optimal brain performance. Unlike most vitamins, it isn't widely available in food and is mainly made in the body through sun exposure. Because of this, low levels are common, and maintaining healthy vitamin D is important to support energy, mood, and long-term immune, bone, and cardiovascular health.

**Evidence Level:** ⭐⭐⭐ (3/5)

## Test Information

| Property | Value |
|----------|-------|
| **Category** | Functional Medicine |
| **Subcategory** | Nutrient Panel |
| **Test Type** | Standard |
| **Sample Type** | Blood |
| **Collection Method** | Venipuncture |
| **Turnaround Time** | 3-7 days |
| **Price Range** | $49-$99 |
| **HSA Eligible** | ✅ Yes |
| **FSA Eligible** | ✅ Yes |

## 🧬 Biomarkers Measured

This test measures the following biomarkers with detailed measurement context:

### [[Vitamin D, 25-Hydroxy]] (Primary)

| Property | Value |
|----------|-------|
| **Unit** | ng/mL |
| **Method** | Immunoassay / LC-MS |
| **Conventional Range** | 30 - 100 ng/mL |
| **Optimal Range** | 40 - 70 ng/mL |

**Interpretation Guide:**
**Deficiency:** < 20 ng/mL
**Insufficiency:** 20 - 29 ng/mL
**Optimal:** 40 - 70 ng/mL
**Toxicity:** > 100 ng/mL

Functional medicine practitioners often recommend maintaining levels between 40-70 ng/mL for optimal immune and bone health.

## 🏥 Where to Order

Compare providers for this test:

### 1. Walk-In Lab
- **Price:** $59.00
- **Turnaround:** 2 days
- **Quality Rating:** ⭐⭐⭐⭐
- **[🔗 Order Test](https://www.walkinlab.com/products/view/vitamin-d-25-hydroxy-blood-test)**

### 2. Quest Diagnostics
- **Price:** $69.00
- **[🔗 Order Test](https://www.questhealth.com/product/vitamin-d-test-17306M.html)**

### 3. Labcorp OnDemand
- **Price:** $79.00
- **[🔗 Order Test](https://www.ondemand.labcorp.com/lab-tests/vitamin-d)**

## Recommended For

### Supplement Monitoring
No specific supplement monitoring recommendations.

### Conditions
Useful for monitoring or diagnosing:

- [[Vitamin D Deficiency]]
- [[Bone Loss (Osteopenia Or Osteoporosis)]]
- [[Impaired Immune Function]]
- [[Low Mood/Cognitive Effects]]

## Reference Ranges

### Conventional Reference Range
| Marker | Low | Normal | High |
|--------|-----|--------|------|
| Vitamin D, 25-OH | < 30 | 30 - 100 | > 100 |

### Optimal/Functional Range
For functional medicine interpretation, optimal ranges may be narrower than conventional lab ranges.

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- → [[Low_MoodCognitive_Effects]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```
- → [[Vitamin_D_Deficiency]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```
- → [[vitamin_d_25oh]] (labtest)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```
- → [[Vitamin_D_25-Hydroxy]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```
- → [[Bone_Loss_Osteopenia_Or_Osteoporosis]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```
- → [[Impaired_Immune_Function]] (entity)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```
- → [[Vitamin_D_25-Hydroxy]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.620736'
source: obsidian
```
- → [[Low_MoodCognitive_Effects]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.620736'
source: obsidian
```
- → [[Impaired_Immune_Function]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.620736'
source: obsidian
```
- → [[Bone_Loss_Osteopenia_Or_Osteoporosis]] (entity)
```yaml
last_modified: '2026-01-26T07:08:01.620736'
source: obsidian
```
- ← [[vitamin_d_25oh]] (labtest)
```yaml
last_modified: '2026-01-26T07:08:01.541578'
source: obsidian
```
- ← [[Vitamin_D_Deficiency]] (condition)
```yaml
last_modified: '2026-01-26T07:07:39.191758'
source: obsidian
```
- ← [[mthfr_gene]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:05.244890'
source: obsidian
```
- ← [[extended_vitamins_minerals_nutrients]] (labtestpanel)
```yaml
last_modified: '2026-01-26T07:08:03.904495'
source: obsidian
```
- → [[Vitamin_D_Deficiency]] (condition)
```yaml
last_modified: '2026-01-26T07:08:01.620736'
source: obsidian
```
- → [[mthfr_gene]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```
- → [[extended_vitamins_minerals_nutrients]] (labtestpanel)
```yaml
last_modified: '2026-01-26T06:48:29.084652'
source: obsidian
```

### Measures
- → [[Vitamin_D_25-Hydroxy]] (entity)
```yaml
conventional_range: 30 - 100
interpretation_guidance: '**Deficiency:** < 20 ng/mL

  **Insufficiency:** 20 - 29 ng/mL

  **Optimal:** 40 - 70 ng/mL

  **Toxicity:** > 100 ng/mL


  Functional medicine practitioners often recommend maintaining levels between 40-70
  ng/mL for optimal immune and bone health.

  '
is_primary: true
last_modified: '2026-01-26T07:08:01.583515'
measurement_method: Immunoassay / LC-MS
measurement_unit: ng/mL
optimal_range: 40 - 70
source: obsidian
```

### Included In
- → [[mthfr_gene]] (labtestpanel)
- → [[extended_vitamins_minerals_nutrients]] (labtestpanel)
<!-- NEO4J_CONTENT_END -->
## Relationships
No relationships found.

## Notes

- Results should be interpreted by a qualified healthcare provider
- Reference ranges may vary by laboratory
- Individual factors (age, sex, health status) affect interpretation

---
*Last Updated: 2026-01-19T09:05:39.783464+00:00*