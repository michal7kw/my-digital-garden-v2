---
created: '2026-02-13T19:00:15.525721Z'
description: Encodes Presenilin-1, the catalytic subunit of the Gamma-Secretase complex.
  Mutations are the most common cause of Familial Early-Onset Alzheimer's.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/gene/psen1/
slug: psen1
tags:
- gene
- alzheimers
- genetics
- protease
templateEngineOverride: njk
title: PSEN1
type: gene
updated: '2026-02-13T19:00:15.525721Z'
---

{% raw %}
<h1>PSEN1</h1>
<h2>Overview</h2>
<p>The <strong>PSEN1</strong> gene encodes <strong>Presenilin-1</strong>, a multi-pass transmembrane protein that functions as the catalytic core of the <strong>Gamma-Secretase</strong> complex. This protease complex is responsible for the intramembrane cleavage of several proteins, most notably the <strong>Amyloid Precursor Protein (APP)</strong> and <strong>Notch</strong> receptors.</p>
<p>Mutations in PSEN1 are the most common cause of <strong>Familial Alzheimer's Disease (FAD)</strong>, a rare, early-onset form of the disease that is fully penetrant (if you have the mutation, you will develop the disease).</p>
<h2>Biological Functions</h2>
<h3>1. Gamma-Secretase Activity</h3>
<p>Presenilin-1 provides the active site (two aspartate residues) for gamma-secretase. This enzyme cleaves APP to generate Amyloid-Beta (Aβ) peptides.</p>
<ul>
<li><strong>Normal Function:</strong> Produces a mix of Aβ40 (shorter, less toxic) and Aβ42 (longer, aggregation-prone).</li>
</ul>
<h3>2. Notch Signaling</h3>
<p>Cleaves Notch receptors to release the Notch Intracellular Domain (NICD), which translocates to the nucleus to regulate developmental genes. Crucial for neurogenesis and cell differentiation.</p>
<h3>3. Calcium Homeostasis</h3>
<p>Regulates calcium leak from the endoplasmic reticulum (ER). Mutations can disrupt this, leading to calcium overload and neuronal toxicity.</p>
<h2>Clinical Significance</h2>
<ul>
<li><strong>Familial Alzheimer's Disease (FAD):</strong> Over 200 different mutations in PSEN1 have been identified.
<ul>
<li><strong>Mechanism:</strong> Most mutations shift the cleavage site, increasing the ratio of <strong>Aβ42 to Aβ40</strong>. Aβ42 aggregates much more rapidly into toxic oligomers and plaques.</li>
<li><strong>Onset:</strong> Typically very early, often between ages 30 and 50.</li>
</ul>
</li>
<li><strong>Acne Inversa:</strong> Rare mutations are linked to this chronic skin condition.</li>
</ul>
<h2>Relationships</h2>
<p>ENCODES::<a class="internal-link is-unresolved" href="/404">Presenilin-1 Protein</a><br />
PART_OF::<a class="internal-link is-unresolved" href="/404">Gamma-Secretase Complex</a> - Catalytic subunit<br />
CLEAVES::<a class="internal-link is-unresolved" href="/404">Amyloid Precursor Protein</a> - Substrate<br />
CLEAVES::<a class="internal-link is-unresolved" href="/404">Notch Receptor</a> - Substrate<br />
REGULATES::<a class="internal-link is-unresolved" href="/404">Amyloid Beta</a> - Production ratio<br />
ASSOCIATED_WITH_CONDITION::<a class="internal-link is-unresolved" href="/404">Familial Alzheimer's Disease</a> - Primary cause<br />
ASSOCIATED_WITH_CONDITION::<a href="/garden/dev_admin/condition/alzheimers-disease/" class="internal-link">Alzheimer's Disease</a><br />
INTERACTS_WITH::<a class="internal-link is-unresolved" href="/404">PSEN2</a> - Functional paralog (Presenilin-2)<br />
INTERACTS_WITH::<a class="internal-link is-unresolved" href="/404">Nicastrin</a> - Complex partner</p>
<h2>References</h2>
<ul>
<li>Sherrington R, et al. Cloning of a gene bearing missense mutations in early-onset familial Alzheimer's disease. Nature. 1995;375(6534):754-60.</li>
<li>De Strooper B, et al. Deficiency of presenilin-1 inhibits the normal cleavage of amyloid precursor protein. Nature. 1998;391(6665):387-90.</li>
</ul>
<!-- NEO4J_CONTENT_START -->
<h2>Relationships</h2>
<h3>Related</h3>
<ul>
<li>← <a href="/garden/dev_admin/protein/presenilin-1/" class="internal-link">Presenilin-1</a> (protein)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T06:49:02.579296'
source: obsidian
</code></pre>
<ul>
<li>→ <a class="internal-link is-unresolved" href="/404">Familial_Alzheimers_Disease</a> (entity)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T06:48:04.131236'
source: obsidian
</code></pre>
<ul>
<li>→ <a href="/garden/dev_admin/pathway/amyloid-cascade/" class="internal-link">Amyloid_Cascade</a> (entity)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T06:48:04.131236'
source: obsidian
</code></pre>
<ul>
<li>→ <a href="/garden/dev_admin/protein/presenilin-1/" class="internal-link">Presenilin-1</a> (protein)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T07:07:46.822035'
source: obsidian
</code></pre>
<ul>
<li>→ <a class="internal-link is-unresolved" href="/404">Familial_Alzheimers_Disease</a> (entity)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T07:07:46.822035'
source: obsidian
</code></pre>
<ul>
<li>← <a href="/garden/dev_admin/pathway/amyloid-cascade/" class="internal-link">Amyloid_Cascade</a> (pathway)</li>
</ul>
<pre><code class="language-yaml">last_modified: '2026-01-26T07:08:11.155642'
source: obsidian
</code></pre>
<!-- NEO4J_CONTENT_END -->
{% endraw %}