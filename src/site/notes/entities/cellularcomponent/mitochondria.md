---
created: '2026-02-08T19:25:12.909683Z'
description: Mitochondria are double-membrane-bound organelles found in most eukaryotic
  organisms. They generate most of the cell's supply of adenosine triphosphate (ATP),
  used as a source of chemical energy, through cellular respiration.
dg-home: false
dg-publish: true
gardenUsername: dev_admin
permalink: /garden/dev_admin/CellularComponent/mitochondria/
slug: mitochondria
tags:
- CellularComponent
- cellular_component
- organelle
- energy_production
title: Mitochondria
type: CellularComponent
updated: '2026-02-08T19:25:12.909683Z'
---

# Mitochondria

## Description

Mitochondria are double-membrane-bound organelles found in most eukaryotic organisms. They generate most of the cell's supply of adenosine triphosphate (ATP), used as a source of chemical energy, through cellular respiration.

## Structure

- **Outer membrane**: Permeable to small molecules
- **Intermembrane space**: Contains cytochrome c
- **Inner membrane**: Contains electron transport chain complexes
- **Matrix**: Contains TCA cycle enzymes, mtDNA

## Key Metabolic Functions

### Energy Production
- **[[Citric Acid Cycle]]**: Oxidizes acetyl-CoA to CO₂, generates NADH/FADH₂
- **[[Electron Transport Chain]]**: Transfers electrons, pumps protons
- **[[Oxidative Phosphorylation]]**: ATP synthesis via [[ATP Synthase]]
- **[[Beta-Oxidation]]**: Fatty acid catabolism for acetyl-CoA

### Key Metabolites
- **[[ATP]]**: Primary energy output
- **[[NADH]]** / **[[NAD+]]**: Electron carriers
- **[[FADH2]]**: Electron carrier for Complex II
- **[[Acetyl-CoA]]**: Central metabolic hub
- **[[Pyruvate]]**: Glycolysis endpoint, enters as acetyl-CoA

### Regulation of Biogenesis
- **[[PGC-1alpha]]**: Master regulator of mitochondrial biogenesis
- **[[TFAM]]**: mtDNA replication and transcription
- **[[NRF2]]**: Antioxidant defense
- **[[AMPK]]**: Energy sensor, activates biogenesis
- **[[SIRT1]]** / **[[SIRT3]]**: NAD+-dependent regulation

## Supplement Support

| Supplement | Mechanism |
|------------|-----------|
| **[[CoQ10]]** | Electron carrier (Complex I → III) |
| **[[PQQ]]** | Induces biogenesis via PGC-1α |
| **[[NMN]]** / **[[Nicotinamide Riboside]]** | NAD+ precursors for sirtuins |
| **[[Alpha Lipoic Acid]]** | PDH/α-KGDH cofactor |
| **[[Creatine]]** | ATP buffer via phosphocreatine |
| **[[Resveratrol]]** | SIRT1 activation |
| **[[Berberine]]** | AMPK activation |

## Clinical Relevance

- **Aging**: Mitochondrial dysfunction central to aging
- **Neurodegeneration**: High neuronal energy demands
- **Metabolic disease**: Impaired oxidative capacity
- **Exercise adaptation**: Biogenesis enhances capacity

## Recent Research (2024-2026)

- **Sleep:** A 2025 study identified mitochondrial "electrical stress" (electron leak) as the direct driver of sleep pressure in the brain. [[Nature 2025 Sleep and Mitochondria]]
- **Uncoupling:** The uncoupler BAM15 was shown to reverse sarcopenic obesity and extend lifespan by mimicking exercise. [[2025 BAM15 Mitochondrial Uncoupling Review]]
- **Transplantation:** First human trials of "Mitotherapy" (mitochondrial transplantation) began in late 2025. [[2025 Mitrix Bio Mitochondrial Transplantation Trial]]
- **Stress Control:** miR-71 identified as a master regulator of mitochondrial stress signaling. [[Nat Comm 2026 miR-71 Mitochondrial Stress]]

<!-- NEO4J_CONTENT_START -->
## Relationships
### Related
- ← [[Creatine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:41.099790'
source: obsidian
```
- ← [[Resveratrol]] (supplement)
```yaml
last_modified: '2026-01-26T06:42:53.933138'
source: obsidian
```
- ← [[PQQ]] (supplement)
```yaml
last_modified: '2026-01-26T06:42:53.580760'
source: obsidian
```
- ← [[Alpha_Lipoic_Acid]] (supplement)
```yaml
last_modified: '2026-01-26T06:42:47.821091'
source: obsidian
```
- ← [[TFAM]] (protein)
```yaml
last_modified: '2026-01-26T06:49:05.170821'
source: obsidian
```
- ← [[SIRT3]] (protein)
```yaml
last_modified: '2026-01-26T06:49:04.570766'
source: obsidian
```
- ← [[SIRT1]] (protein)
```yaml
last_modified: '2026-01-26T06:49:04.523353'
source: obsidian
```
- ← [[PGC-1alpha]] (protein)
```yaml
last_modified: '2026-01-26T06:49:02.391047'
source: obsidian
```
- ← [[NRF2]] (protein)
```yaml
last_modified: '2026-01-26T06:49:02.167395'
source: obsidian
```
- ← [[ATP_Synthase]] (protein)
```yaml
last_modified: '2026-01-26T06:48:59.343642'
source: obsidian
```
- ← [[AMPK]] (protein)
```yaml
last_modified: '2026-01-26T06:48:58.583262'
source: obsidian
```
- ← [[Oxidative_Phosphorylation]] (pathway)
```yaml
last_modified: '2026-01-26T06:48:41.845781'
source: obsidian
```
- ← [[Citric_Acid_Cycle]] (pathway)
```yaml
last_modified: '2026-01-26T06:48:40.589405'
source: obsidian
```
- ← [[Beta-Oxidation]] (pathway)
```yaml
last_modified: '2026-01-26T06:48:40.393431'
source: obsidian
```
- ← [[Pyruvate]] (metabolite)
```yaml
last_modified: '2026-01-26T06:48:34.727553'
source: obsidian
```
- ← [[Nicotinamide_Riboside]] (metabolite)
```yaml
last_modified: '2026-01-26T06:48:34.490822'
source: obsidian
```
- ← [[NADH]] (metabolite)
```yaml
last_modified: '2026-01-26T06:48:34.411960'
source: obsidian
```
- ← [[NAD]] (metabolite)
```yaml
last_modified: '2026-01-26T06:48:34.302675'
source: obsidian
```
- ← [[FADH2]] (metabolite)
```yaml
last_modified: '2026-01-26T06:48:33.885647'
source: obsidian
```
- ← [[Acetyl-CoA]] (metabolite)
```yaml
last_modified: '2026-01-26T06:48:33.375061'
source: obsidian
```
- ← [[Berberine]] (drug)
```yaml
last_modified: '2026-01-26T06:47:56.150744'
source: obsidian
```
- ← [[Keto_Diet]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:48.239770'
source: obsidian
```
- ← [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2026-01-26T07:07:48.659175'
source: obsidian
```
- ← [[ATP]] (metabolite)
```yaml
last_modified: '2026-01-26T07:08:05.909377'
source: obsidian
```
- ← [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2026-01-26T06:42:49.159387'
source: obsidian
```
- ← [[Magnesium]] (supplement)
```yaml
last_modified: '2026-01-26T07:08:38.494293'
source: obsidian
```
- ← [[NMN]] (supplement)
```yaml
last_modified: '2026-01-26T07:08:39.445715'
source: obsidian
```
- ← [[Oxidative_Stress]] (condition)
```yaml
last_modified: '2026-01-26T07:07:34.918464'
source: obsidian
```
- ← [[Electron_Transport_Chain]] (pathway)
```yaml
last_modified: '2026-01-26T06:48:40.766680'
source: obsidian
```
- → [[NAD]] (metabolite)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[ATP_Synthase]] (entity)
```yaml
last_modified: '2026-01-26T06:47:26.144799'
source: obsidian
```
- → [[Nicotinamide_Riboside]] (entity)
```yaml
last_modified: '2026-01-26T06:47:26.144799'
source: obsidian
```
- → [[Alpha_Lipoic_Acid]] (entity)
```yaml
last_modified: '2026-01-26T06:47:26.144799'
source: obsidian
```
- → [[Citric_Acid_Cycle]] (entity)
```yaml
last_modified: '2026-01-26T06:47:26.144799'
source: obsidian
```
- → [[NAD]] (entity)
```yaml
last_modified: '2026-01-26T06:47:26.144799'
source: obsidian
```
- → [[Oxidative_Phosphorylation]] (entity)
```yaml
last_modified: '2026-01-26T06:47:26.144799'
source: obsidian
```
- → [[AMPK]] (protein)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Nicotinamide_Riboside]] (metabolite)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[lab_results_2025_05_01]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Coenzyme_Q10]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[SIRT1]] (protein)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[supplement_regimen]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[CoQ10]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[FADH2]] (metabolite)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Electron_Transport_Chain]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[SIRT3]] (protein)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Pyruvate]] (metabolite)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[ATP_Synthase]] (protein)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Oxidative_Phosphorylation]] (pathway)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Red_Light_Therapy]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Citric_Acid_Cycle]] (pathway)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[NRF2]] (protein)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[PGC-1alpha]] (protein)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[health_goals]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Keto_Diet]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[TFAM]] (protein)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Acetyl-CoA]] (metabolite)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Oxidative_Stress]] (entity)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[NADH]] (metabolite)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Beta-Oxidation]] (pathway)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[NMN]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Magnesium]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Creatine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[ATP]] (metabolite)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Nicotine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Mitophagy]] (pathway)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[PQQ]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Electron_Transport_Chain]] (pathway)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Resveratrol]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Berberine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```
- → [[Alpha_Lipoic_Acid]] (supplement)
```yaml
last_modified: '2026-01-26T07:07:19.549791'
source: obsidian
```

### Produces
- → [[ATP]] (metabolite)
```yaml
last_modified: '2025-12-28T11:01:06.517949'
source: obsidian
```

### Affected By Intervention
- → [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:06.571468'
source: obsidian
```

### Affected By Diet
- → [[Keto_Diet]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:06.639479'
source: obsidian
```

### Improves Cellular Component
- ← [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2026-01-19T12:00:43.836954'
source: obsidian
```
- ← [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:10.475736'
source: obsidian
```

### Has Mechanism Of Action
- ← [[Creatine]] (drug)
```yaml
last_modified: '2025-12-28T11:01:08.518365'
source: obsidian
```

### Affects Cellular Component
- ← [[Keto_Diet]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:10.394807'
source: obsidian
```

### Supports
- ← [[Magnesium]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:22.653067'
source: obsidian
```
- ← [[Creatine]] (drug)
```yaml
last_modified: '2026-01-26T07:08:36.518401'
source: obsidian
```
- ← [[NMN]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:23.753510'
source: obsidian
```

### Involves
- ← [[Oxidative_Stress]] (condition)
```yaml
last_modified: '2026-01-19T09:37:22.629336'
source: obsidian
```

### Affects
- ← [[Nicotine]] (drug)
```yaml
last_modified: '2026-01-26T07:07:42.749430'
source: obsidian
```
- ← [[Mitophagy]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:12.248727'
source: obsidian
```

### Located In
- ← [[Electron_Transport_Chain]] (pathway)
```yaml
last_modified: '2026-01-26T07:08:11.837447'
source: obsidian
```
<!-- NEO4J_CONTENT_END -->
## Relationships

### Involves
- ← [[Oxidative_Stress]] (condition)
```yaml
last_modified: '2026-01-19T09:31:31.414383'
source: obsidian
```

### Affects
- ← [[Nicotine]] (drug)
```yaml
last_modified: '2026-01-19T09:32:01.142183'
source: obsidian
```
- ← [[Mitophagy]] (pathway)
```yaml
last_modified: '2026-01-19T09:33:13.751724'
source: obsidian
```

### Located In
- ← [[Electron_Transport_Chain]] (pathway)
```yaml
last_modified: '2026-01-19T09:33:12.833714'
source: obsidian
```

### Produces
- → [[ATP]] (metabolite)
```yaml
last_modified: '2025-12-28T11:01:06.517949'
source: obsidian
```

### Affected By Intervention
- → [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:06.571468'
source: obsidian
```
- → [[Mitochondrial Uncoupling]] (intervention)
- → [[Mitochondrial Transplantation]] (intervention)

### Affected By Diet
- → [[Keto_Diet]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:06.639479'
source: obsidian
```

### Improves Cellular Component
- ← [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2026-01-19T09:31:54.373077'
source: obsidian
```
- ← [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:10.475736'
source: obsidian
```

### Has Mechanism Of Action
- ← [[Creatine]] (drug)
```yaml
last_modified: '2025-12-28T11:01:08.518365'
source: obsidian
```

### Affects Cellular Component
- ← [[Keto_Diet]] (intervention)
```yaml
last_modified: '2025-12-28T11:01:10.394807'
source: obsidian
```

### Supports
- ← [[CoQ10]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:19.740004'
source: obsidian
```
- ← [[Magnesium]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:22.653067'
source: obsidian
```
- ← [[NMN]] (supplement)
```yaml
last_modified: '2025-12-28T11:01:23.753510'
source: obsidian
```

### Related
- ← [[health_goals]] (entity)
```yaml
last_modified: '2026-01-19T08:42:06.512352'
source: obsidian
```
- ← [[lab_results_2025_05_01]] (entity)
```yaml
last_modified: '2026-01-19T08:42:07.040190'
source: obsidian
```
- ← [[supplement_regimen]] (entity)
```yaml
last_modified: '2026-01-19T08:42:07.311290'
source: obsidian
```
- → [[Red_Light_Therapy]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Keto_Diet]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Coenzyme_Q10]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[health_goals]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[lab_results_2025_05_01]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[supplement_regimen]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Oxidative_Stress]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Nicotine]] (drug)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Mitophagy]] (pathway)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Electron_Transport_Chain]] (entity)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- ← [[Oxidative_Stress]] (condition)
```yaml
last_modified: '2026-01-19T09:31:31.681292'
source: obsidian
```
- → [[NMN]] (supplement)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Magnesium]] (supplement)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[CoQ10]] (supplement)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[Creatine]] (drug)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- → [[ATP]] (metabolite)
```yaml
last_modified: '2026-01-19T09:30:34.809082'
source: obsidian
```
- ← [[Creatine]] (drug)
```yaml
last_modified: '2026-01-19T09:31:54.843123'
source: obsidian
```
- ← [[Electron_Transport_Chain]] (pathway)
```yaml
last_modified: '2026-01-19T09:33:13.103750'
source: obsidian
```
- ← [[Keto_Diet]] (intervention)
```yaml
last_modified: '2026-01-19T09:32:10.258074'
source: obsidian
```
- ← [[Red_Light_Therapy]] (intervention)
```yaml
last_modified: '2026-01-19T09:32:11.579627'
source: obsidian
```
- ← [[ATP]] (metabolite)
```yaml
last_modified: '2026-01-19T09:32:55.212976'
source: obsidian
```
- ← [[Coenzyme_Q10]] (supplement)
```yaml
last_modified: '2026-01-18T17:28:10.254635'
source: obsidian
```
- ← [[CoQ10]] (supplement)
```yaml
last_modified: '2026-01-19T09:07:00.698822'
source: obsidian
```
- ← [[Magnesium]] (supplement)
```yaml
last_modified: '2026-01-19T09:07:07.990267'
source: obsidian
```
- ← [[NMN]] (supplement)
```yaml
last_modified: '2026-01-19T09:07:11.744171'
source: obsidian
```